...
class ControllerCommonColumnLeft extends Controller {
	public function index() {
		if (isset($this->request->get['user_token']) && isset($this->session->data['user_token']) && ($this->request->get['user_token'] == $this->session->data['user_token'])) {
			...
			// favourite_link
			$favourite_link = array();
			
			$user_token = $this->session->data['user_token'];
			
			$link_data = ($this->config->get('module_favourite_link'));

			if(!empty($link_data)){
				$sort_order = array();
				
				foreach ($link_data as $key => $row)
				{
					$sort_order[$key] = $row['sort_order'];
				}
				array_multisort($sort_order, SORT_ASC, $link_data);

				for ($i=0; $i < count($link_data); $i++) { 
					$old_link = explode('&',$link_data[$i]['link']);
					foreach($old_link as $key => $value)
					{
						$old_link[1] = 'amp;user_token='.$user_token;
					}
					$new_link=join("&",$old_link);
					if ($this->user->hasPermission('access', 'extension/module/favourite_link')) {
						$favourite_link[] = array(
							'name'	   => $link_data[$i]['title'],
							'href'     => $new_link,
							'children' => array()
						);
					}
			...
			if ($favourite_link) {
				$data['menus'][] = array(
					'id'       => 'menu-report',
					'icon'	   => 'fa fa-heart',
					'name'	   => $this->language->get('text_favourite_link'),
					'href'     => '',
					'children' => $favourite_link
				);
			}
			...
			if ($this->user->hasPermission('access', 'extension/module/favourite_link')) {
				$system[] = array(
					'name'	   => $this->language->get('text_favourite'),
